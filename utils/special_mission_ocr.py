...
class OCREngine:
    ...
    def ocr_missions(
        self, search_area, search_text=None, click=False, scroll=False, scrap=False
    ):
        """
        ...
        if self.scroll:
            MouseControllerMixin(x, y, mouse_click=False)
        ...
        while attempts <= max_attempts:
            try:
                ...
                cell_result = self.get_reader().readtext(
                    image=enhanced_screenshot, **ocr_params
                )
                if self.scrap:
                    if cell_result:
                ...
                for mission, (bounding_box, text, _) in enumerate(cell_result):
                    ...
                    if temp_img in text:
                        ...
                        if self.click:
                            MouseControllerMixin(center_x, center_y, mouse_click=True)
