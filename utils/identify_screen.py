...
class IdentifyScreen:
	...
	def draw_bounding_boxes(self, image_array, found_filenames):
		if found_filenames:  # Display image with bounding boxes only if there are detected objects
			for filename, confidence in found_filenames:
				...
				cv.putText(image_array, f"{filename} - {confidence:.2f}", (top_left[0], top_left[1] - 10),
		...
		try:
			...
			with ThreadPoolExecutor() as executor:
				...
				for filename, needle_img in self.needle_images.items():
					...
					futures.append(
						executor.submit(self.match_template, image_array, needle_img, filename)
					)
