...
class DbConfig:
    ...
    def load_search_areas_to_db(self):
        ...
        search_areas = [
            SearchArea(
                region_name="active_missions", left=530, top=830, right=909, bottom=1017
            ),
            SearchArea(
                region_name="mission_package_area",
                left=1221,
                top=724,
                right=1470,
                bottom=756,
            ...
            SearchArea(
                region_name="scrollable_area", left=890, top=275, right=1040, bottom=505
            ),
        ]
    ...
    def load_characters_to_db(self):
        with Session(engine) as session:
            ...
            for character_name, character_data in var.items():
                ...
                character = Character(
                    username=character_name, email=email, password=pwd
                )
                session.add(character)
            session.commit()
