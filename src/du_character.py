...
class DUCharacters:
    ...
    def login(self, character):
        ...
        with multiprocessing.Pool(processes=2) as pool:
            du_login_screen_label_result = pool.apply_async(
                get_du_login_screen_label, (self.verify,)
            )
            loading_complete_result = pool.apply_async(
                get_loading_complete, (self.verify,)
            )

            (
                success_du_login_screen_label,
                screen_coords_du_login_screen_label,
                minSearchTime_du_login_screen_label,
            ) = du_login_screen_label_result.get()
            (
                success_loading_complete,
                screen_coords_loading_complete,
                minSearchTime_loading_complete,
            ) = loading_complete_result.get()
        ...
        while count <= attempts:
            ...
            with multiprocessing.Pool(processes=5) as pool:
                connection_queued_result = pool.apply_async(
                    get_connection_queued, (self.verify,)
                )
                # email_login_result = pool.apply_async(get_email_login, (self.verify,))
                internal_error_result = pool.apply_async(
                    get_internal_error, (self.verify,)
                )
                gametime_error_lable_result = pool.apply_async(
                    get_gametime_error_lable, (self.verify,)
                )
                password_error_result = pool.apply_async(
                    get_password_error, (self.verify,)
                )

                (
                    success_connection_queued,
                    screen_coords_connection_queued,
                    minSearchTime_connection_queued,
                ) = connection_queued_result.get()
                # success_email_login, screen_coords_email_login, minSearchTime_email_login = email_login_result.get()
                (
                    success_internal_error,
                    screen_coords_internal_error,
                    minSearchTime_internal_error,
                ) = internal_error_result.get()
                (
                    success_gametime_error_lable,
                    screen_coords_gametime_error_lable,
                    minSearchTime_gametime_error_lable,
                ) = gametime_error_lable_result.get()
                (
                    success_password_error,
                    screen_coords_password_error,
                    minSearchTime_password_error,
                ) = password_error_result.get()
            ...
            if success_internal_error:
                ...
                logger.warning(
                    f"Password error: character {character.email} deactivated"
                )
                return False
            ...
            # 	continue
                ...
                logger.success(
                    f"Connection successful: character {character.email} active"
                )
    ...
    def check_location(self):  # TODO: Implement
        ...
        # find Market icon
        market_icon = self.verify.screen(
            screen_name=ImageLocation.MAP_SCREEN,
