...
class VerifyScreen:
    ...
    def screen(
        ...
        self.image_data = ImageQuerySet.read_image_by_name(
            image_name=image_to_compare, image_location=screen_name
        )
        ...
        while max_checks >= check_count:
            ...
            if (
                screen_coords := pyautogui.locateOnScreen(
                    image=os.path.join(
                        DirectoryPaths.ROOT_DIR, self.image_data.image_url
                    ),
                    # region=region,
                ...
                check_count += 1
                logger.info(
                    f"looking... {image_to_compare}, {screen_name} {check_count}/{max_checks}"
                )
                continue
