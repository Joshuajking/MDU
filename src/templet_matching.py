...
def template_matching(image_to_compare):
    ...
    try:
        ...
        if max_val >= threshold:
            print(
                f"Found needle: {image_data} - {max_val} match | threshold: {threshold}"
            )
            ...
            print(
                f"Needle not found: {image_data} - {max_val} match | threshold: {threshold}"
            )
...
if __name__ == "__main__":
    convert_to_32_bit()
    template_matching(
        image_to_compare=r"C:\Repositories\Dual Universe\DualUniverse_Projects\MDU\data\images\geforce_screen\geforce_select_game.png"
    )
